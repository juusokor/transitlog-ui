...
export default ({filter, children}) => {
  ...
  if (filter.type === 'line') {
      return <Query client={joreClient} query={routesByLineQuery} variables={filter}>
        {({ loading, error, data }) => children({ loading, error, data: (data && data.line) ? data.line.routes.nodes : data })}
      </Query>;
  } else {
      return children({loading: false, error: new Error('unknown filter type'), data: null});
  }
};
